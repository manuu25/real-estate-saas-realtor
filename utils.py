...
except Exception:  # noqa: BLE001 - embedchain has native deps (chromadb) that may be absent.
    App = None
import os
import requests
import tempfile
...
load_dotenv()
# En Streamlit Cloud los secretos llegan via st.secrets; los replicamos a os.environ
# para que el codigo (que usa os.environ) funcione igual en local y en la nube.
try:
...
# Endpoint S3-compatible (Backblaze B2 / Storj / R2). Vacio = AWS S3 nativo.
S3_ENDPOINT_URL = os.environ.get('S3_ENDPOINT_URL') or None
S3_REGION = os.environ.get('AWS_DEFAULT_REGION') or None
...
def is_email_subscribed(email):
    ...
    if not stripe.api_key:
        # Sin Stripe configurado: no bloquea (la suscripcion no restringe el uso).
        return False
    try:
        ...
        print(f"[aviso] Stripe no disponible ({_stripe_err}); se continua sin suscripcion.")
        return False
...
def create_bot(selected_address, selected_tenant):
    if App is None:
        st.error(
            "La funcion de chat requiere 'embedchain', que no esta instalado "
            "(necesita chromadb / compilador C++). Instala MS C++ Build Tools o "
            "usa un venv con Python 3.11 y `pip install embedchain==0.1.43`."
        )
        st.stop()
